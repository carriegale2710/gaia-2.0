...
This footer is mandatory on **every** PR. **Never** omit it, **never** replace it with plain text such as `*Created by GAIA 2.0*`, and **never** drop the 🌱 emoji or the `[GAIA 2.0](https://gaiacode.pro)` link. Full rules live in `SYSTEM_PROMPT.md`; this gate-level statement overrides any conflicting impulse.
